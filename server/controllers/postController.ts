...
const pollLeonardoJob = async (generationId: string, apiKey: string): Promise<string> => {
    ...
    for (let i = 0; i < maxRetries; i++) {
        ...
        await new Promise(() => setTimeout(resolve, delay));
    }
...
export const generatePost = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const textResponse = await ai.models.generateContent({
            ...
            The "imagePrompt" should be a highly descriptive prompt for an image generation that complementd the post.`
        });
        let content = "";
            ...
            try {
                ...
                if (leonardoKey) {
                    ...
                    const leoResponse = await axios.post(
                        "https://cloud.leonardo.ai/api/v2/generations",
                        {
                            "public": false,
                            "model": "gpt-image-2",
                            "parameters": {
